...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // LOAD ROOMS + UNIT TYPES + RESERVATIONS
    ...
    const created = [];
    const groupId = roomDefs.length > 1 ? Math.random().toString(16).slice(2, 10) : "";

    for (let ri = 0; ri < roomDefs.length; ri++) {
      const rd = roomDefs[ri];
      ...
      if (groupId) notes += " | Gruppe " + groupId + " (Zi " + (ri + 1) + "/" + roomDefs.length + ")";
      ...
      created.push({ id: nr[0].id, room: free.name, roomType: ut.name, price: rd.price });

      // Zahlung anlegen wenn Booking Online
      if (paymentMethod) {
        try {
          await sbPost("payments", {
            reservation_id: nr[0].id,
            guest_id: guestId,
            amount: rd.price,
